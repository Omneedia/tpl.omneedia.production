...
function read_registry(cb) {
    fs.readFile(__dirname + require('path').sep + '..' + require('path').sep + 'registry.json', function(e, r) {
        if (e) global.registry = process.env;
        else global.registry = JSON.parse(r.toString('utf-8'));
        cb();
    });
};
...
function read_manifest(cb) {
    fs.readFile(__dirname + require('path').sep + 'app.manifest', function(e, r) {
        if (e) return new Error('MANIFEST_NOT_FOUND');
        else {
...
function master(err, port) {
    ...
    socket.on('REGISTER_WORKER', function(config) {
        fs.writeFile(__dirname + path.sep + 'etc' + path.sep + 'settings.json', JSON.stringify(config), function() {
        ...
        */
        ...
        if (reg_session.indexOf('mongodb://') > -1) {
            ...
            app.use(session({
                ...
                cookie: {
                    path: '/',
                    domain: global.settings.url.substr(global.settings.url.indexOf('.'), 255),
...
function start() {
    ...
    if (cluster.isMaster) freeport(master);
    else server();
